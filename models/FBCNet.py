    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.lastLayer = self.LastBlock(
            self.m * self.nBands * self.strideFactor, nClass, doWeightNorm=doWeightNorm
        )

    def forward(self, x):
        # N, B, C, T
        if (
            self.data_name == "BNCI2015001"
            or self.data_name == "MI1-7"
            or self.data_name == "MI1"
        ):
            x = x[:, :-1, :]
        N, C, T = x.shape
        ...
        if (
            "BNCI2014001" in self.data_name
            or self.data_name == "BNCI2014002"
            or self.data_name == "BNCI2015001"
        ):
            x = x[:, :, :, :-2]
