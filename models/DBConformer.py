...
class Stem(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.data_name != "MI1-7":  # MI1-7 is the Blankertz2007 dataset in our paper
            out = out[
                :, :, :-1
            ]  # Example: 14001 has 1001 time points, we exclude the final point
...
class PatchEmbeddingSpatial(nn.Module):
    def __init__(self, spa_dim, emb_size=40):
        ...
        self.encoder = nn.Sequential(
            nn.Conv1d(
                1, spa_dim, kernel_size=25, stride=5, padding=12
            ),  # Output: (B*C, spa_dim, T')
            nn.ELU(),
...
class DBConformer(nn.Module):
    def __init__(
        self, args, emb_size=40, tem_depth=5, chn_depth=5, chn=22, n_classes=2
    ):
        super().__init__()
    ...
    def forward(self, x):  # x: (B, 1, C, T)
        ...
        if self.posemb_flag:
            x_embed = (
                x_embed + self.pos_embedding_temporal
            )  # temporal positional encoding
            x_embed_spatial = (
                x_embed_spatial + self.pos_embedding_spatial
        ...
        if self.branch == "temporal":
            ...
            if self.gate_flag:
                ...
                x_fused = gate * x_spatial.mean(dim=1) + (1 - gate) * x_temporal.mean(
                    dim=1
                )
            else:
