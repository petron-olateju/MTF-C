...
class FBMSNet(nn.Module):
    ...
    def LastBlock(self, inF, outF, doWeightNorm=True, *args, **kwargs):
        return nn.Sequential(
            LinearWithConstraint(
                inF, outF, max_norm=0.5, doWeightNorm=doWeightNorm, *args, **kwargs
            ),
            nn.LogSoftmax(dim=1),
        )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.scb = self.SCB(
            in_chan=num_Feat, out_chan=num_Feat * dilatability, nChan=int(nChan)
        )
    ...
    def forward(self, x):
        ...
        input("")
        x = x.reshape(
            [*x.shape[0:2], self.strideFactor, int(x.shape[3] / self.strideFactor)]
        )
...
class FBMSNet_Inception(nn.Module):
        ...
        **kwargs,
        ...
        self.Tception1 = nn.Sequential(
            nn.Conv2d(
                9,
                9,
                kernel_size=(1, int(self.inception_window[0] * sampling_rate)),
                stride=1,
                padding=(0, int(self.inception_window[0] * sampling_rate / 2)),
            ),
            nn.BatchNorm2d(9),
        )
        self.Tception2 = nn.Sequential(
            nn.Conv2d(
                9,
                9,
                kernel_size=(1, int(self.inception_window[1] * sampling_rate - 1)),
                stride=1,
                padding=(0, int(self.inception_window[1] * sampling_rate / 2 - 1)),
            ),
            nn.BatchNorm2d(9),
        )
        self.Tception3 = nn.Sequential(
            nn.Conv2d(
                9,
                9,
                kernel_size=(1, int(self.inception_window[2] * sampling_rate)),
                stride=1,
                padding=(0, int(self.inception_window[2] * sampling_rate / 2)),
            ),
            nn.BatchNorm2d(9),
        )
        self.Tception4 = nn.Sequential(
            nn.Conv2d(
                9,
                9,
                kernel_size=(1, int(self.inception_window[3] * sampling_rate)),
                stride=1,
                padding=(0, int(self.inception_window[3] * sampling_rate / 2)),
            ),
            nn.BatchNorm2d(9),
        )
    ...
    def forward(self, x):
        ...
        out = self.scb(out)
        out = out.reshape(
            [*out.shape[0:2], self.strideFactor, int(out.shape[3] / self.strideFactor)]
        )
