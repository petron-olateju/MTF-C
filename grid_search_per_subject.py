...
def main():
    ...
    if args.dataset == "all":
        raise ValueError(
            "--dataset argument has to be single dataset when running grid search per subject"
        )
    device = args.device
    ...
    for combo in tqdm(combos, total=total_combos, desc="Grid Search"):
        ...
        if script == "cross_validation":
            accuracy, kappa, stft_reconstruction_loss, experiment, _ = cross_validation(
                mthd_args, experiment, config="grid_search", model_configs=current_param
            )

            if args.model_name != "mtf_c":
                print(
                    f"subject {args.subject} | Accuracy: {np.mean(accuracy):.2f}, Kappa: {np.mean(kappa):.2f}"
                )
                print("============================================")
            else:
                print(
                    f"subject {args.subject} | Accuracy: {np.mean(accuracy):.2f}, Kappa: {np.mean(kappa):.2f} | STFT Reconstruction Loss: {np.mean(stft_reconstruction_loss):.2f}"
                )
                print("============================================")
    ...
    if experiment is not None:
        ...
        run_entry = {
            ...
            "subject": args.subject,
            "metrics": {
                "accuracy": {"mean": float(best_acc), "std": float(best_acc_std)},
                "kappa": {"mean": float(best_kappa), "std": float(best_kappa_std)},
